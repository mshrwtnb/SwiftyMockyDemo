...
open class UserRepositoryMock: UserRepository, Mock {
    ...
    fileprivate enum MethodType {
        ...
        static func compareParameters(lhs: MethodType, rhs: MethodType, matcher: Matcher) -> Bool {
            switch (lhs, rhs) {
            case (.m_find__by_id(let lhsId), .m_find__by_id(let rhsId)):
    ...
    open class Given: StubbedMethod {
        ...
        public static func find(by id: Parameter<Int>, willProduce: (Stubber<User?>) -> Void) -> MethodStub {
            let willReturn: [User?] = []
			let given: Given = { return Given(method: .m_find__by_id(`id`), products: willReturn.map({ StubProduct.return($0 as Any) })) }()
